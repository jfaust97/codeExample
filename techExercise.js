// tech exercise script for hypothes.is


$('#newNoteInput').keyup(function(e) {
      thisElementID = 'newNoteInput';
      loadJSON(thisElementID);
});

$('#newNoteInput').click(function(e) {
      thisElementID = 'newNoteInput';
      var currentText = $('#'+thisElementID).html();
});
...
function clearNewComment(){
	newNoteValue = $('#newNoteInput').text();
	if (newNoteValue == 'Add new comment...') {
		$('#newNoteInput').text('');
	};
};
...
function insertName(name){
  $('#nameWidget').remove();
  var newNoteInput = $('#newNoteInput').html().toString();
  newNoteInput = newNoteInput.slice(0, -4);
  newNoteInput += " " + name;
  $('#newNoteInput').html('');
  $('#newNoteInput').html(newNoteInput);
}

function createWidget(thisElementID,newList){
  var makeList = "";

  for (x in newList) {
     if (newList.hasOwnProperty(x)) {
       newLink = newList[x].avatar_url;
       makeList += ('<div class="avatar_url" id="avatar_url'+newList[x].id+'"><img src="'+newLink+'" class="avatarImg"></div><div class="userName" onclick="insertName(\''+newList[x].name +'\')">' + newList[x].name + '</div><div class="clearBoth"></div>');
     }
  }

  $('#nameWidget').remove();
  var newWidget = '';
  newWidget += '<div class="nameWidget" id="nameWidget">';
  newWidget += makeList;
  newWidget += '</div>';
  $('#newNameListAnchor').append(newWidget);
};




function removeWidget(thisElementID,newList){
  $('#nameWidget').remove();
}



function loadJSON(thisElementID) {
  var items = [];
  var data = [
  {
    "username": "pturner0",
  ...
  },
  {
    ...
    var currentText = $('#'+thisElementID).html();
    var lastFour = currentText.substr(currentText.length - 4);

    if(lastFour.length > 3){
      for(var i = 0; i < data.length; i++) {
        var obj = data[i];
        var newList = data.filter(element => {
            for (var property in element) {
                    if(element[property].indexOf(lastFour) >= 0) {
                        return true;
                    };
            };
        });
      ...
      if(newList.length > 0 && newList != undefined){
        createWidget(thisElementID,newList);
      }else{
        removeWidget(thisElementID,newList);
      }
    }
};
